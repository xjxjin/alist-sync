...
import re
import schedule
from alist_sync import main  # 假设你的main函数在这个模块里



# 解析命令行参数
parser = argparse.ArgumentParser(description="文件同步脚本，支持cron语法的定时任务。")
parser.add_argument('--cron_schedule', type=str, required=True, metavar='CRON表达式',
                    help="CRON表达式，用于定时执行任务，例如：'* 21 2 * *'")
...
parser.add_argument('--cron_schedule', type=str, required=True, metavar='cron语法定时调度',
                    help="cron语法定时调度，默认为  * 21 2 * *")
...
# 使用传入的cron表达式安排任务
schedule.schedule_cron(job_id='main_job', schedule=args.cron_schedule, func=run_task)
